<div>
    <x-molecules.card title="Data Jenis Supplier">
        <x-slot name="toolbar">
            <x-atoms.button.btn-modal target="#modal_form">New Data</x-atoms.button.btn-modal>
        </x-slot>
        <livewire:datatables.jenis-supplier-table />
    </x-molecules.card>

    <x-molecules.modal title="Form Jenis Supplier" id="modal_form" size="lg" wire:ignore.self>
        <form>
            <x-atoms.input.group label="Jenis" required="required">
                <x-atoms.input.text name="jenis" wire:model.defer="jenis" />
            </x-atoms.input.group>
